...
function setupKnob(knob, container) {
  ...
  if ('ontouchstart' in window) {
    ...
    document.addEventListener('mousemove', function(e) {
      ...
      updateMotorValue(knob.element.id, knob.element.value)
...
var CSRFToken;
    ...
    this.updateMotorValue = async function (knob, value) {
        try {
            const response = await fetch("/knobChanged", {
                method: 'POST',
                mode: 'cors',
                cache: 'no-cache',
                ...
                body: JSON.stringify({"knob": knob, "value": value})
            })
